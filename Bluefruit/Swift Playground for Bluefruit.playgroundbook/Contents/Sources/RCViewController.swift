...
//        printLog("keyboardWillHide")
    ...
    @objc func onTouchUpClear(_ sender: UIButton) {
        sendTouchEvent(sender.tag, isPressed: true)
        logView.text = ""
    }
    
    @objc func onTouchDownForward(_ sender: UIButton) {
        sendTouchEvent(sender.tag, isPressed: true)
        let isPressed = true
        rcBluetooth.moveForward()
    }
    
    @objc func onTouchUpForward(_ sender: UIButton) {
        let isPressed = false
        sendTouchEvent(sender.tag, isPressed: true)
        rcBluetooth.stopForward()
    }
    
    @objc func onTouchDownBack(_ sender: UIButton) {
        sendTouchEvent(sender.tag, isPressed: true)
        let isPressed = true
    ...
    @objc func onTouchUpBack(_ sender: UIButton) {
        sendTouchEvent(sender.tag, isPressed: true)
        let isPressed = false
    ...
    @objc func onTouchDownRight(_ sender: UIButton) {
        sendTouchEvent(sender.tag, isPressed: true)
        let isPressed = true
    ...
    @objc func onTouchUpRight(_ sender: UIButton) {
        let isPressed = false
        rcBluetooth.stopRight()
    }
    
    
    @objc func onTouchDownLeft(_ sender: UIButton) {
        sendTouchEvent(sender.tag, isPressed: true)
        let isPressed = true
    ...
    @objc func onTouchUpLeft(_ sender: UIButton) {
        let isPressed = false
...
//        printLog("--> adding assessment val \(command)")
    ...
    func UISetup() {
        ...
        clearButton.setImage(image, for: UIControl.State.normal)
        ...
        forwardButton.setTitleColor(buttonEnabledColor, for: UIControl.State.selected)
        ...
        backButton.setTitleColor(buttonEnabledColor, for: UIControl.State.selected)
        ...
        leftButton.setTitleColor(buttonEnabledColor, for: UIControl.State.selected)
        ...
        rightButton.setTitleColor(buttonEnabledColor, for: UIControl.State.selected)
...
//        view.addConstraints(generateConstraintsForCarImageView(toView: view))
        ...
        self.view.backgroundColor = #colorLiteral(red: 0.2862745098, green: 0.6885969606, blue: 0.8901960784, alpha: 1)
...
//    class ConnectionViewDelegate: PlaygroundBluetoothConnectionViewDelegate, PlaygroundBluetoothConnectionViewDataSource {
        ...
        public func connectionView(_ connectionView: PlaygroundBluetoothConnectionView, titleFor state: PlaygroundBluetoothConnectionView.State) -> String {
            ...
            case .selectingPeripherals:
                return NSLocalizedString("Select your robot", comment: "")
            case .connectedPeripheralFirmwareOutOfDate:
                return NSLocalizedString("Connect to a different robot", comment: "")
            }
        }
...
//        printLog("received: \(message)")
        ...
        if rcBluetooth.isConnected == false {
            printLog("Connect to robot before sending commands")
            exitProgram()
            return
        }
